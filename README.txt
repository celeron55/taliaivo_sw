...
- Mainly just to see that Piston is behaving. Heap allocations aren't used on
  the embedded target.
$ cd taliaivo_simulator
$ cargo build
$ valgrind --tool=massif ../target/debug/taliaivo_simulator
...
$ cd taliaivo_simulator
$ cargo build
$ valgrind --tool=massif --heap=no --stacks=yes ../target/debug/taliaivo_simulator
$ ms_print massif.out.<pid> | less
...
$ ./dfu_release.sh
- OR
$ dfu-util -a 0 --dfuse-address 0x08000000  -D ../target/thumbv7em-none-eabihf/release/taliaivo_embedded.bin
...
In another terminal:
$ cd taliaivo_embedded
$ arm-none-eabi-gdb ../target/thumbv7em-none-eabihf/release/taliaivo_embedded
